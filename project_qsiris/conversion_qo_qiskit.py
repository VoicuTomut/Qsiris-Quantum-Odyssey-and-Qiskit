    ...
    if len(moment.control_q) == 0:
        ...
        for qubit in range(moment.nr_q):
            ...
            if gate_name == "X":
                ...
                ########################################################
                qiskit_circuit = qiskit_circuit + 'qc.unitary(unit, qubits, label="{}") \n'.format(moment.original_form[qubit]["GateDefinition"]["Name"].lower())
                ########################################################
...
def odyssey_to_qiskit(puzzle, incl_initial_state = False,
    ...
    PuzzleGates=np.reshape(puzzle['PuzzleGateSlots'],(puzzle['PuzzleDefinition']['QubitCapacity'],
                                        int(len(puzzle['PuzzleGateSlots'])/puzzle['PuzzleDefinition']['QubitCapacity'])))
    
    for puzzle_gate in conv._transpose_list(PuzzleGates):
        if use_barrier:
            qc.barrier()
            ########################################################
